...
class NeuralNet: #nom de la class à changer
    ...
    def train(self, train, train_labels,nb_classes,learning_rate,nb_epoch):
        """
        c'est la mthode qui va entrainer votre modle,
        train est une matrice de taille nxm, avec 
        n : le nombre d'exemple d'entrainement dans le dataset
        m : le mobre d'attribus (le nombre de caractristiques)
        ...
        if self.nb_attributs > 2: self.liste_couches.append(0.05*np.random.randn(self.nb_attributs,self.nb_neurones))
    ...
    def test(self, test, test_labels):
        """
        c'est la mthode qui va tester votre modle sur les donnes de test
        ...
        Faites le test sur les donnes de test, et afficher :
        - la matrice de confision (confusion matrix)
        - l'accuracy (ou le taux d'erreur)
        
        Bien entendu ces tests doivent etre faits sur les donnes de test seulement
    ...
    def encode(self):
        nn_file = open('neuralnet_saved.txt','w')
    ...
    def decode(self):
        nn_file = open('neuralnet_saved.txt','r')
        ...
        for i in range(self.nb_neurones):
            for j in range(self.nb_attributs):
                ...
                compteur+=1
        for k in range(1,self.nb_couches-1):
